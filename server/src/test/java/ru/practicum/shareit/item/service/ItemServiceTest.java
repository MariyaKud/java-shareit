...
class ItemServiceTest {
    ...
    ItemDtoOutWithBookings itemDtoWithBooking;

    Comment comment;
    CommentDtoIn commentDto;

    CommentDtoOut commentDtoOut;

    Booking booking;

    BookingDtoOutShort bookingDtoOut;
    ...
    void beforeEach() {
        ...
        commentDto = new CommentDtoIn(comment.getText());

        commentDtoOut = new CommentDtoOut(comment.getId(), comment.getText(),
        ...
        bookingDtoOut = BookingDtoOutShort.builder()
                .id(1L)
        ...
        itemDtoWithBooking = ItemDtoOutWithBookings.builder()
                .id(item.getId())
                .name(item.getName())
                .description(item.getDescription())
                .available(item.getAvailable())
    ...
    void should_create_comment() {
        ...
        CommentDtoOut newComment = itemService.createComment(guest.getId(), item.getId(), commentDto, current);
    ...
    void get_items_by_user_id() {
        ...
        List<ItemDtoOutWithBookings> items = itemService.getItemsByUserId(user.getId(), 0, 10);
